...
def read_training_files(data_path:str):
    ...
    # combine train and validation and let keras split it randomly
    tv_enc = t_enc + v_enc
...
# Flatten list [[a,b], [c,d], [e,f], [g,h]] -> [[a,b,c,d], [e,f,g,h]] -> [a,b,c,d,e,f,g,h]
...
def get_step2_prerequisite_files(output_directory):
    ...
    assert not list(outpath.glob('*')), f'There are files in {str(outpath.absolute())}, please clear\'' \
                                        f'files or specify an empty folder.'
